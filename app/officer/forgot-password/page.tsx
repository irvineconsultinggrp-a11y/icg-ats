  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: passwordResetRedirect(origin, "/officer/reset-password"),
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white px-6">
      <div className="w-full max-w-md flex flex-col gap-6">
        <Link href="/officer/login" className="text-sm text-[#6b7280] hover:text-[#061c2a]">
          ← Back to sign in
        </Link>
        <h1 className="text-2xl font-bold text-[#061c2a]">Reset officer password</h1>
